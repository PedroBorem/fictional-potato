...
| 92 | Reinicio do modem / ACK generico | O modem publica o motivo e reinicia o ESP32 apos timer; na placa existe ACK local e, no fluxo do heartbeat, esse IDP tambem e usado para pedir o reset do modem antes de considerar um reset da placa. | `subscricao padrao -> cloudv2-info` | `modem` | No `fw_ICrop` e no `fw_outorga` tambem publica a mensagem de reset em topico de rede e reinicia o sistema. | `Placa -> modem local: #92-PIVO_ID$; modem/rede: #92-PIVO_ID-MOTIVO$` |
