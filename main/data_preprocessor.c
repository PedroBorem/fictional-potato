...
static app_callback data_preprocessor_callback = NULL;
...
/**
 * @brief Convert a model probability or threshold to protocol scale.
 *
 * @param value Floating-point value in the range 0.0 to 1.0.
 *
 * @return Value multiplied by 10000 and clamped to the uint16_t range.
 */
static uint16_t probability_to_protocol_scale(double value)
{
    if (value <= 0.0) {
        return 0U;
    }
    ...
    return (uint16_t)((value * 10000.0) + 0.5);
}

/**
 * @brief Convert a feature value to uint8_t for the IDP 67 event.
 *
 * @param value Feature value generated by preprocessing.
 *
 * @return Rounded and clamped uint8_t value.
 */
static uint8_t feature_to_uint8(float value)
{
    if (value <= 0.0f) {
        return 0U;
    }
...
/**
 * @brief Convert a feature value to uint16_t for the IDP 67 event.
 *
 * @param value Feature value generated by preprocessing.
 *
 * @return Rounded and clamped uint16_t value.
 */
static uint16_t feature_to_uint16(float value)
{
    if (value <= 0.0f) {
        return 0U;
    }
...
/**
 * @brief Create and dispatch the internal IDP 67 event packet.
 *
 * The packet is routed through the callback registered by the system manager,
 * keeping the same dispatch architecture used by other internal modules.
 *
 * @param probability_scaled Error probability multiplied by 10000.
 * @param threshold_scaled Decision threshold multiplied by 10000.
 * @param center_packet Raw center packet evaluated by the model.
 * @param date_ts Timestamp generated from the center packet date.
 */
static void dispatch_ai_error_event(
    uint16_t probability_scaled,
    uint16_t threshold_scaled,
    const data_preprocessor_raw_packet_t *center_packet,
    uint32_t date_ts)
{
    if (data_preprocessor_callback == NULL || center_packet == NULL) {
        return;
    }
    ...
    char date_ts_str[16] = {};
    uint8_t idp = IDP_67;
    const char *event_type = "AI_ERROR";

    snprintf(source_idp_str, sizeof(source_idp_str), "%02" PRIu8, feature_to_uint8(center_packet->idp));
    snprintf(d_str, sizeof(d_str), "%" PRIu8, feature_to_uint8(center_packet->d));
    snprintf(w_str, sizeof(w_str), "%" PRIu8, feature_to_uint8(center_packet->w));
    snprintf(p_str, sizeof(p_str), "%" PRIu8, feature_to_uint8(center_packet->p));
    snprintf(percent_str, sizeof(percent_str), "%" PRIu16, feature_to_uint16(center_packet->percent));
    snprintf(ang_str, sizeof(ang_str), "%" PRIu16, feature_to_uint16(center_packet->ang));
    snprintf(date_ts_str, sizeof(date_ts_str), "%" PRIu32, date_ts);

    arg_pair_t arg_pairs[] = {
        {"uint8_t", &idp},
        {"string", (void *)event_type},
        {"uint16_t", &probability_scaled},
        {"uint16_t", &threshold_scaled},
        ...
        {"string", ang_str},
        {"string", date_ts_str},
        {"string", (void *)center_packet->date},
        {NULL, NULL}};

    idp_parser_create_package(str_out, arg_pairs);
    data_preprocessor_callback(str_out, COMM_MQTT);
}
...
/**
 * @brief Convert a named feature row to the exact model input order.
 *
 * @param features Source feature row.
 * @param feature_vector Destination vector expected by the m2cgen model.
 */
static void feature_row_to_model_vector(
    const data_preprocessor_feature_row_t *features,
    double feature_vector[DATA_PREPROCESSOR_FEATURE_COUNT])
{
    feature_vector[0] = (double)features->d_act;
    feature_vector[1] = (double)features->w_act;
    feature_vector[2] = (double)features->p_act;
    feature_vector[3] = (double)features->perc_act;
    feature_vector[4] = (double)features->idp;
    feature_vector[5] = (double)features->d;
    feature_vector[6] = (double)features->w;
    feature_vector[7] = (double)features->p;
    feature_vector[8] = (double)features->percent;
    feature_vector[9] = (double)features->ang;
    feature_vector[10] = (double)features->date_ts;
    feature_vector[11] = (double)features->sin_ang;
    feature_vector[12] = (double)features->cos_ang;
    feature_vector[13] = (double)features->ang_is_calculating;
    feature_vector[14] = (double)features->window_lag5_p_changed_centered_11;
    feature_vector[15] = (double)features->window_lag5_w_changed_centered_11;
    feature_vector[16] = (double)features->window_lag5_d_changed_centered_11;
    feature_vector[17] = (double)features->window_lag5_idp_30_count_centered_11;
    feature_vector[18] = (double)features->window_lag5_total_time_delta_sec_centered_11;
    feature_vector[19] = (double)features->window_lag5_has_action_centered_11;
    feature_vector[20] = (double)features->window_pos5_p_changed_centered_11;
    feature_vector[21] = (double)features->window_pos5_w_changed_centered_11;
    feature_vector[22] = (double)features->window_pos5_d_changed_centered_11;
    feature_vector[23] = (double)features->window_pos5_idp_30_count_centered_11;
    feature_vector[24] = (double)features->window_pos5_total_time_delta_sec_centered_11;
    feature_vector[25] = (double)features->window_pos5_has_action_centered_11;
    feature_vector[26] = (double)features->window_lag5_delta_percent_centered_11;
    feature_vector[27] = (double)features->window_lag5_range_percent_centered_11;
    feature_vector[28] = (double)features->window_lag5_max_delta_percent_centered_11;
    feature_vector[29] = (double)features->window_lag5_delta_ang_centered_11;
    feature_vector[30] = (double)features->window_lag5_range_ang_centered_11;
    feature_vector[31] = (double)features->window_lag5_max_delta_ang_centered_11;
    feature_vector[32] = (double)features->window_pos5_delta_percent_centered_11;
    feature_vector[33] = (double)features->window_pos5_range_percent_centered_11;
    feature_vector[34] = (double)features->window_pos5_max_delta_percent_centered_11;
    feature_vector[35] = (double)features->window_pos5_delta_ang_centered_11;
    feature_vector[36] = (double)features->window_pos5_range_ang_centered_11;
    feature_vector[37] = (double)features->window_pos5_max_delta_ang_centered_11;
}
...
/**
 * @brief Registers the callback used to route generated IDP packets.
 *
 * @param callback Callback provided by the system manager.
 */
void data_preprocessor_register_callback(const app_callback callback)
{
    data_preprocessor_callback = callback;
}

/**
 * @brief Calculate the center feature row for the current centered window.
 *
 * @param features Destination feature row.
 *
 * @return ESP_OK when filled, ESP_ERR_INVALID_ARG when features is NULL, or
 * ESP_ERR_INVALID_STATE when the centered window is not ready.
 */
    ...
    data_preprocessor_feature_row_t *features)
{
    ...
    copy_ring_ordered(ordered);
    calculate_features_for_row(ordered, HALF_WINDOW, features);

    return ESP_OK;
}

/**
 * @brief Fill the model input vector for the current centered window.
 *
 * @param feature_vector Destination vector in XGBoost feature order.
 *
 * @return ESP_OK when filled, ESP_ERR_INVALID_ARG when feature_vector is NULL,
 * or ESP_ERR_INVALID_STATE when the centered window is not ready.
 */
    ...
    double feature_vector[DATA_PREPROCESSOR_FEATURE_COUNT])
{
    ...
    data_preprocessor_feature_row_t features;
    const esp_err_t err = data_preprocessor_get_center_feature_row(&features);

    if (err != ESP_OK) {
        return err;
    }

    feature_row_to_model_vector(&features, feature_vector);

    return ESP_OK;
}

/**
 * @brief Run the embedded XGBoost model for the current center packet.
 *
 * This function keeps inference ownership inside the preprocessing module. The
 * system manager only feeds raw packets; once the centered window is ready, the
 * preprocessor builds the 38-feature vector, runs the model and logs only
 * positive error detections.
 */
static void run_error_inference_for_center_packet(void)
{
    data_preprocessor_feature_row_t center_features;
    buffer_packet_t ordered[DATA_PREPROCESSOR_WINDOW_SIZE];
    double feature_vector[DATA_PREPROCESSOR_FEATURE_COUNT];
    ...
    copy_ring_ordered(ordered);
    calculate_features_for_row(ordered, HALF_WINDOW, &center_features);
    feature_row_to_model_vector(&center_features, feature_vector);

    const double error_probability =
        xgboost_predict_error_probability(feature_vector);
    const bool has_error = error_probability >= XGBOOST_ERROR_THRESHOLD;

    if (has_error) {
        ESP_LOGW(DATA_PREPROCESSOR_TAG,
                 "ERRO ENCONTRADO | probabilidade=%.4f | threshold=%.2f | resultado=ERRO",
                 error_probability,
                 (double)XGBOOST_ERROR_THRESHOLD);
        ESP_LOGW(DATA_PREPROCESSOR_TAG, "Linha central enviada ao modelo:");
        print_features_header();
        print_features_row(&center_features);

        const data_preprocessor_raw_packet_t *center_packet = &ordered[HALF_WINDOW].packet;
        const uint16_t probability_scaled = probability_to_protocol_scale(error_probability);
        const uint16_t threshold_scaled = probability_to_protocol_scale(XGBOOST_ERROR_THRESHOLD);
        const uint32_t date_ts =
            ordered[HALF_WINDOW].date_ts > UINT32_MAX ? UINT32_MAX : (uint32_t)ordered[HALF_WINDOW].date_ts;

        dispatch_ai_error_event(
            probability_scaled,
            threshold_scaled,
            center_packet,
            date_ts);
...
esp_err_t data_preprocessor_add_raw_packet(const data_preprocessor_raw_packet_t *raw_packet)
{
    ...
    if (data_preprocessor_is_window_ready()) {
        ...
        run_error_inference_for_center_packet();
    }
