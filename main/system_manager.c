...
void system_manager_init(void)
{
	...
	eco_mode_config eco_config = {};
	data_app_load(DATA_TYPE_ECO_MODE_CONFIG, &eco_config);
	eco_mode_start(eco_config);
...
/**
 * @brief Suspends Eco Mode for the current window when a command turns the pivot on.
 *
 * Any command that results in a non-off power state disables Eco Mode for the current
 * configured window. The rush override notification is emitted only when the pivot was
 ...
 * @param ignore_override True when the command itself was generated by Eco Mode.
 */
static void system_manager_handle_eco_mode_override(const pivot_actions *new_actions, const pivot_actions *old_actions, bool ignore_override)
{
	if (ignore_override == false && new_actions != NULL && old_actions != NULL && new_actions->power_state != PIVOT_OFF)
	{
...
static void system_manager_idp_30(const char *buffer, comm_type comm_mode)
{
	...
	if (ret == ESP_OK)
	{
		...
		system_manager_handle_eco_mode_override(&new_actions, &old_actions, false);
