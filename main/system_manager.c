...
static void system_manager_idp_67(const char *buffer, comm_type comm_mode);
...
void system_manager_init(void)
{
	...
	data_preprocessor_register_callback(&system_manager_callback);
...
static void system_manager_callback(const char *buffer_request, comm_type comm_mode)
{
	...
	if (payload_ascii_valid == true)
	{
		...
		case IDP_31:
		{
			...
			case IDP_67:
			{
				system_manager_idp_67(str_pkg, comm_mode);
				break;
			}
			case IDP_90:
			{
...
/**
 * @brief Handles IDP 67 AI error events generated by the preprocessor.
 *
 * The preprocessor emits an internal IDP 67 packet through the registered
 * callback. This handler receives that packet through system_manager_callback,
 * adds the current system_id and publishes the final event via MQTT.
 *
 * Internal packet:
 * #67-AI_ERROR-probability_x10000-threshold_x10000-source_idp-D-W-P-PERCENT-ANG-DATE_TS-DATE$
 *
 * Published packet:
 * #67-system_id-AI_ERROR-probability_x10000-threshold_x10000-source_idp-D-W-P-PERCENT-ANG-DATE_TS-DATE$
 *
 * @param buffer The internal IDP 67 packet.
 * @param comm_mode The communication mode used by the callback.
 */
static void system_manager_idp_67(const char *buffer, comm_type comm_mode)
{
	(void)comm_mode;
	...
	char date_ts_str[16] = {};
	char str_date_time[50] = {};
	uint8_t idp_in = IDP_67;
	uint8_t idp_out = IDP_67;
	uint16_t probability_scaled = 0;
	uint16_t threshold_scaled = 0;

	arg_pair_t arg_pairs_in[] = {
		{"uint8_t", &idp_in},
		{"string", event_type},
		{"uint16_t", &probability_scaled},
		{"uint16_t", &threshold_scaled},
		{"string", source_idp_str},
		{"string", d_str},
		{"string", w_str},
		{"string", p_str},
		{"string", percent_str},
	...
	idp_parser_get_packet_data(buffer, arg_pairs_in);
